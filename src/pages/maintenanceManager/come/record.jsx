...
import { Form, Input, Button, Select, Tooltip, Table, Pagination, DatePicker,Modal,TreeSelect } from 'antd'
  ...
  outAdd=()=>{
    ...
    this.formRef.current.validateFields().then(async values => {
      ...
      var promes={
        ...
        ck_money:values.ck_money,
        ck_reason:values.ck_reason,
    ...
    return (
      <div className="classStorage">
              ...
              <Form.Item className="searchBtn">
                ...
                <Button  type="primary" onClick={() => {
                  this.setState({
                      ...
                      setTimeout(()=>{
                        this.formRef.current.setFieldsValue({flag:'1'})
                      },200)
                    })
                  }}>新增</Button>
              </Form.Item>
        ...
        <Modal
          ...
          onOk={() => this.outAdd()}
          onCancel={() => { this.formRef.current.resetFields();this.setState({ visibleAdd: false,goodsnum:null }) }}
          width={520}
        >
          <div>
            <Form ref={this.formRef}  {...formItemLayout}  >
              ...
              <Form.Item rules={[{ required: true, message: '请选择物料名称!' }]} name="goods_id" label="物料名称">
                <Select placeholder="请选择物料名称" onChange={(e)=>{
                  this.formRef.current.setFieldsValue({goods_num:null})
                  this.setState({
              ...
              <div style={{position:'relative'}}>
                    <Form.Item rules={[{ required: true, message: '请输入换新数量!' },({ getFieldValue }) => ({
                  validator(_, value) {
                    if (value > goodsnum) {
                ...
                }),]} name="goods_num" label="数量">
                <Input placeholder="请输入换新数量" style={{ width: 250 }} type="number"></Input>
              </Form.Item>
                <span style={{position:'absolute',right:'55px',top:'7px'}}>
                    {goodsnum===null?"":<span style={{fontSize:'13px',color:'rgb(198, 199, 199)'}}>剩余{goodsnum}</span>}
                </span>
                </div>
              <Form.Item  name="ck_money" label="换新费用">
                <Input placeholder="请输入换新费用" style={{ width: 250 }} type="number"></Input>
              </Form.Item>
              <Form.Item  name="ck_reason" label="维修原因">
