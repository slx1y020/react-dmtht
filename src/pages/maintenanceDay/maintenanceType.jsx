...
import { Form, Input, Button, Select, Table, Pagination, Modal, DatePicker, Popconfirm, message, Radio, InputNumber,Tooltip } from 'antd'
import { req_findWorkUser, req_findSpaceFloorRoom, req_addMaintenancetask, req_findAllMaintenancetask, req_deleteMaintenancetask } from '../../api/hdlApi'
...
class Storage extends Component {
  ...
  // 查询角色信息
  fetch = async pageNum => {
    const { rooms, maintainUser, state, pageSize, taskMethod,task_schedule } = this.state
    const params = {
      rooms,
      maintainUser,
      state,
    ...
    const res = await req_findAllMaintenancetask(params)
    if (res.code === 20000) {
      // console.log(res.data.list,'sssssssssssssssssss')
      this.setState({
        data: res.data.list,
  ...
  handleAdd = () => {
    setTimeout(() => {
      ...
      this.formRef.current.setFieldsValue({ task_method: 1 });
    }, 10)
  ...
  userIdClick = async (val) => {
    ...
    if (val !== undefined) {
      ...
      const res = await req_findSpaceFloorRoom({ userId: val })
      if (res.code === 20000) {
        this.setState({
          spaceRoomData: res.data?res.data:[],
        })
  ...
  // 保存
  handleOk = () => {
    const { maintenanceModeId } = this.state;
    this.formRef.current.validateFields().then(async values => {
      ...
      values.task_method = 1
      const res = await req_addMaintenancetask(values)
      if (res.code === 20000) {
  ...
  // 删除
  delete = async (id) => {
    const res = await req_deleteMaintenancetask({ id })
    if (res.code === 20000) {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: taskSchedule => taskSchedule === 1 ? '日维护' : taskSchedule === 2 ? '周维护' : taskSchedule === 3 ? '月维护' : taskSchedule === 0 ?'指定日期':''
      },
      {
        title: '维护状态',
        align: 'center',
        ...
        render: (state) => {
          ...
          if (state === 0) {
            return (<div style={{ color: '#FA8F1B' }}> ● 待维护</div>)
          } else if (state === 1) {
            return (<div style={{ color: '#0FC26E' }}> ● 维护中</div>)
          } else {
            return (<div style={{ color: '#B2B2B2' }}> ● 已完成</div>)
          }
        }
      },
      {
      ...
      },
      {
        ...
        render: record => (
          <div>
            <Popconfirm title="确定要删除吗" okText="确认" cancelText="取消" onConfirm={() => this.delete(record.id)}>
      ...
      <div className="classStorage">
          ...
          <Form layout="inline">
            <div>
              <Form.Item label="维护人姓名	 : ">
                <Input style={{ width: 160 }} placeholder="请输入维护人姓名" onChange={e => this.setState({ maintainUser: e.target.value })} />
              </Form.Item>
              <Form.Item label="房间 : ">
        ...
        >
          <Form {...layout} ref={this.formRef}>
            <Form.Item label="姓名" name="maintain_id" rules={[{ required: true, message: '请选择姓名!' }]}>
              <Select
                placeholder="请选择姓名"
                allowClear
                showSearch
            ...
            <Form.Item label="房间" name="rooms" rules={[{ required: true, message: '请选择房间!' }]}>
              <Select allowClear mode="multiple" placeholder='请选择房间' showSearch optionFilterProp="children">
                {
                  spaceRoomData.map((val, key) => {
                    return (<Option value={val.classroom_code ? val.space_name + val.classroom_code : val.space_name} key={key} disabled={!spaceRoomData.length}>{val.classroom_code ? val.space_name + val.classroom_code : val.space_name}</Option>)
                  })
                }
              </Select>
            </Form.Item>
            ...
            {
              maintenanceModeId === 1 ? <Form.Item label="维护时间" name="date" rules={[{ required: true, message: '请选择维护时间!' }]}>
                <RangePicker showTime style={{ width: '100%' }} />
              </Form.Item> : <Form.Item label="维护周期" name="task_schedule" rules={[{ required: true, message: '请选择维护时间!' }]}>
                  <Radio.Group>
            ...
            <Form.Item label="维护描述" name="task_desc" rules={[{ required: false, message: '请输入维护描述!' }]}>
