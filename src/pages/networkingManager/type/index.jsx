...
import { req_findAllFeilei, req_saveFenlei, req_updateFenlei, req_deleteFenlei} from './../../../api/hdlApi'
...
export default class EquipmentType extends Component {
  ...
  findAllFeilei=(pageNum)=>{
    ...
    req_findAllFeilei({pageNum,pageSize,sb_fl_name}).then(res=>{
      if(res.code===20000){
      this.setState({
        data:res.data.list,
  ...
  //删除类型
  delete=(record)=>{
    req_deleteFenlei({sb_fl_id:record.sb_fl_id}).then(res=>{
      if(res.code===20000){
  ...
  saveFenlei=()=>{
    this.formRef.current.validateFields().then(async values => {
      req_saveFenlei({...values}).then(res=>{
        if(res.code===20000){
  ...
  updateFenlei=()=>{
    ...
    this.formRef.current.validateFields().then(async values => {
      req_updateFenlei({sb_fl_id,...values}).then(res=>{
        if(res.code===20000){
          this.setState({visible:false})
  ...
  render() {
    ...
    const columns = [
      ...
      {
        title: '分类图标',
        dataIndex: 'sb_fl_icon',
        render:(icon)=>{
          return <a href={IP.hostImg + icon} target="_blank"> 
              <img src={ IP.hostImg + icon } style={{width:'40px'}} alt='' />
            </a>
          }
      },
      {
    ...
    return (
      <div className="classroomType">
        <div className="topForm">
          <Form layout="inline">
            <div>
              <Form.Item name="sb_fl_name" label="网络设备类型 : ">
                <Input onChange={(e)=>this.setState({sb_fl_name:e.target.value})} style={{ width: 160 }} placeholder="请输入网络设备类型 " />
              </Form.Item>
            </div>
            <div>
              <Form.Item  >
